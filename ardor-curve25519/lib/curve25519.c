 ...
 */
void keygen(BYTE *P, BYTE *s, BYTE *k) {
    clamp(k);
    core(P, s, k, NULL);
}
 ...
 */
void curve(BYTE *Z, BYTE *k, BYTE *P) {
    core(Z, NULL, k, P);
}
 ...
 */
int sign(BYTE *v, BYTE *h, BYTE *x, BYTE *s) {
    // v = (x - h) s  mod q
    int w, i;
    BYTE h1[32], x1[32];
 ...
 *   P  [in]  public key
 */
void verify(BYTE *Y, BYTE *v, BYTE *h, BYTE *P) {
        /* Y = v abs(P) + h G  */
        BYTE d[32];
...
int isCanonicalSignature(BYTE *v) {
        BYTE vCopy[32];
...
int isCanonicalPublicKey(BYTE *publicKey) {
...
char * GetSignPrikey(char *arg) {
    BYTE P[32], s[32], k[32];
    hexStringToByteArray(k, arg);
    keygen(P, s, k);
...
char * GetAgreementPrikey(char *arg) {
    BYTE P[32], k[32];
    hexStringToByteArray(k, arg);
    keygen(P, NULL, k);
...
char * GetAgreementPubkey(char *arg) {
    BYTE P[32], k[32];
    hexStringToByteArray(k, arg);
    keygen(P, NULL, k);
...
char * Sign(char *a, char *b, char *c) {
    // signatureHash keyAgreementPrivateKey signingPrivateKey
    BYTE v[32], h[32], x[32], s[32];
    hexStringToByteArray(h, a);
    hexStringToByteArray(x, b);
    hexStringToByteArray(s, c);
    static char res[65];
    if (!sign(v, h, x, s)) {
        return res;
    }
    char vs[65]; byteArrayToHexString(vs, v); // signatureValue
    strcpy(res, vs);
    return res;
}


char * Verify(char *a, char *b, char *c) {
    // "signatureValue signatureHash signingPublicKey"
    BYTE Y[32], v[32], h[32], P[32];
    hexStringToByteArray(v, a);
    hexStringToByteArray(h, b);
    hexStringToByteArray(P, c);
    verify(Y, v, h, P);
    char Ys[65]; byteArrayToHexString(Ys, Y); // keyAgreementPublicKey
    static char res[65];
    strcpy(res, Ys);
    return res;
}

char * GetAgreementKey(char *a, char *b) {
    BYTE Z[32], k[32], P[32];
    hexStringToByteArray(k, a);
    hexStringToByteArray(P, b);
    curve(Z, k, P);
    char Zs[65]; byteArrayToHexString(Zs, Z); // sharedSecret
    static char res[65];
    strcpy(res, Zs);
    return res;
}
